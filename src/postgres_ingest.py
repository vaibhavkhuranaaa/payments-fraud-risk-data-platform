...
def load_file(connection: psycopg.Connection, path: Path, source_sha: str) -> tuple[int, int]:
    rows: list[tuple[str, str, str, str, str, bool, str, str]] = []
    with path.open(newline="", encoding="utf-8") as source:
        ...
        for record in reader:
            ...
            rows.append((event_id(source_sha, record[""]), record["trans_date_trans_time"], record["merchant"], record["category"], record["amt"], record["is_fraud"] == "1", path.name, source_sha))
    with connection.cursor() as cursor:
        cursor.executemany("""
          INSERT INTO risk.events (event_id, event_ts, merchant, category, amount, is_fraud, source_file, source_sha256)
...
def main() -> None:
    ...
    parser.add_argument("--profile", type=Path, default=Path("data/validated/sparkov-source-profile.json"))
    ...
    with psycopg.connect(args.database_url, autocommit=False, row_factory=dict_row) as connection:
        for filename in ("fraudTrain.csv", "fraudTest.csv"):
            source_sha = profile["files"][filename]["sha256"]
            path = args.raw_dir / filename
            with connection.cursor() as cursor:
                cursor.execute("SELECT EXISTS(SELECT 1 FROM risk.ingestion_runs WHERE source_sha256 = %s AND source_file = %s) AS already_loaded", (source_sha, filename))
                if cursor.fetchone()["already_loaded"]:
                    continue
            input_rows, inserted_rows = load_file(connection, path, source_sha)
            with connection.cursor() as cursor:
                cursor.execute("""
                  INSERT INTO risk.ingestion_runs (run_id, source_name, source_version, source_sha256, source_file, input_rows, inserted_rows, status)
                  VALUES (%s, %s, %s, %s, %s, %s, %s, 'completed')
                  ON CONFLICT (source_sha256, source_file) DO NOTHING
                """, (uuid.uuid4(), SOURCE, SOURCE_VERSION, source_sha, filename, input_rows, inserted_rows))
        connection.commit()
